...
| 3 | What Runway is, the platform | KEPT, condensed | Who you are |
| 3 | The pitch pointer, `VISION.md` | MOVED, G2_BOUNCE, held CLAUDE.md over its 150-line cap | `docs/runway/tp-orientation.md`'s Navigation map, "Strategic context, why X exists" row, already present there before this fix |
...
| 47-54 | Fork-checkout config lines, `git config hygiene.remote upstream` and `git config hygiene.trunk runway`, from the same new-checkout step above | MOVED | `docs/runway/git-hooks-sop-reference.md`, which the charter's Repo setup and pointers section sends readers to for git-hooks detail; the config lines live only there now, not restated in Core operating principle 1 |
...
| Old fork line 119: "Full how-to, git-tracked and permanent: `agencyos-operational-efficiency/docs/standards/build-bay-playbook.md`." G2_BOUNCE caught this restated in the new `CLAUDE.md`'s Repo setup and pointers, holding the file over its 150-line cap | Present upstream, `tp-orientation.md`'s "How to actually run the room" section, verbatim | MOVED, the CLAUDE.md bullet dropped | `docs/runway/tp-orientation.md`, "How to actually run the room" section, first line under the heading. This branch's `CLAUDE.md` bullet duplicated a pointer already present there; removing the duplicate, not the pointer itself, is what freed the line |

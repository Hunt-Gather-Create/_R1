...
`VERCEL_GIT_PULL_REQUEST_ID` empty) > default skip.

SKIP stays above everything and the force flag stays above the env checks so
operators keep both manual overrides.

Why the runway trigger also requires an empty PR id: `VERCEL_GIT_COMMIT_REF`
is the HEAD branch name, so a fork PR whose branch happens to be named
`runway` (an accidental fork-sync PR is the realistic shape) would match on
ref alone and force-push prod schema from unmerged code. The deploy that
actually serves prod is the push-triggered branch deploy of upstream
`runway`, and Vercel gives that deploy an empty `VERCEL_GIT_PULL_REQUEST_ID`;
every PR-triggered preview (including all fork PRs) carries the PR's id.
...
| Runway-branch deploy on Hunt-Gather-Create | `VERCEL_ENV=preview`, `VERCEL_GIT_COMMIT_REF=runway`, `VERCEL_GIT_PULL_REQUEST_ID=""`, URL set | **PUSH** | Vercel classifies it preview, but the live Runway app is aliased to it. This is the RW-INC-2026-07-27-01 gap. | Wrong-skip: exactly RW-INC-2026-07-27-01 — dashboard 500 on first schema-adding PR. |
| Runway-branch deploy, `VERCEL_ENV` missing | `VERCEL_GIT_COMMIT_REF=runway`, no PR id, URL set | **PUSH** | Trigger is env-agnostic on purpose; branch identity is the signal. | Wrong-skip: same as above if Vercel ever omits `VERCEL_ENV`. |
...
| Fork PR whose branch is named `runway` | `VERCEL_ENV=preview`, `VERCEL_GIT_COMMIT_REF=runway`, `VERCEL_GIT_PULL_REQUEST_ID` set | skip | The ref matches but the PR id disqualifies it — PR previews never push. An accidental sync-PR from a fork's `runway` branch is the realistic shape here. | Wrong-push: `drizzle-kit push --force` from a stale fork could DROP tables prod code depends on. |
...
**Known caveat (wrong-skip shape):** if a PR is ever opened FROM the upstream
`runway` branch (e.g. a `runway` → `main` PR on Hunt-Gather-Create), Vercel
links the runway-branch deploys to that PR, the PR id becomes non-empty, and
the trigger skips. If that happens while a schema-adding change ships, set
`RUN_DB_MIGRATIONS=true` on the deploy (or run the push manually per the
incident runbook) and close or retarget the PR. The parity-check line in the
build log (see below) is how you notice.
...
3. You will see exactly one of:
   - `Pushing Runway database schema (<reason>)...` — the gate decided to push.
     ...
     `runway-branch deploy (schema-push contract)`, or
...
4. On a push, also confirm the follow-up line
   `Runway schema parity check passed: ... tables present, _meta seeded ...`.

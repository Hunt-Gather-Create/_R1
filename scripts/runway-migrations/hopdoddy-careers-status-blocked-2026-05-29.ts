...
const PRE_SNAPSHOT_PATH = "docs/tmp/hopdoddy-careers-status-blocked-pre-2026-05-29.json";
const POST_SNAPSHOT_PATH = "docs/tmp/hopdoddy-careers-status-blocked-post-2026-05-29.json";
...
async function preChecks(ctx: MigrationContext): Promise<void> {
  ...
  const [wi] = await ctx.db.select().from(weekItems).where(eq(weekItems.id, CAREERS_WI_ID));
  if (!wi) throw new Error(`Careers Page: Refresh WI (id=${CAREERS_WI_ID}) not found.`);

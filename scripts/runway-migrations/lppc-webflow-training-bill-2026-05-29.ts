...
const NEW_WI = {
  ...
  notes:
    "Walk Bill through Webflow basics so LPPC can self-serve content edits post-launch.",
};

const PRE_SNAPSHOT_PATH =
  "docs/tmp/lppc-webflow-training-bill-pre-2026-05-29.json";
const POST_SNAPSHOT_PATH =
  "docs/tmp/lppc-webflow-training-bill-post-2026-05-29.json";
...
async function preChecks(ctx: MigrationContext): Promise<void> {
  ...
  if (l1.name !== L1_NAME)
    throw new Error(`L1 name drift: '${l1.name}' vs '${L1_NAME}'.`);
  ...
  ctx.log(
    `  Website Revamp L1 ${L1_ID.slice(0, 8)}: status=${l1.status}, dates=${l1.startDate}→${l1.endDate}`
  );
...
async function verify(ctx: MigrationContext): Promise<void> {
  ...
  if (newWi.owner !== NEW_WI.owner)
    throw new Error(`Post: owner drift '${newWi.owner}'.`);

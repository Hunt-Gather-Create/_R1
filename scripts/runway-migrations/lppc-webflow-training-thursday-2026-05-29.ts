...
const PRE_SNAPSHOT_PATH = "docs/tmp/lppc-webflow-training-thursday-pre-2026-05-29.json";
const POST_SNAPSHOT_PATH = "docs/tmp/lppc-webflow-training-thursday-post-2026-05-29.json";
...
export async function up(ctx: MigrationContext): Promise<void> {
  ...
  ctx.log(`--- Step 2a: startDate ${OLD_START} → ${NEW_START} (FIRST — backward) ---`);
  if (!ctx.dryRun) {
...
async function preChecks(ctx: MigrationContext): Promise<void> {
  ...
  const [wi] = await ctx.db.select().from(weekItems).where(eq(weekItems.title, WI_TITLE));
  if (!wi) throw new Error(`WI "${WI_TITLE}" not found.`);
  if (wi.clientId !== client.id) throw new Error(`WI clientId drift.`);
  if (wi.weekOf !== WI_WEEK_OF) throw new Error(`WI weekOf drift: '${wi.weekOf}'.`);
  if (wi.startDate !== OLD_START) throw new Error(`WI startDate drift: '${wi.startDate}'.`);
  if (wi.endDate !== OLD_END) throw new Error(`WI endDate drift: '${wi.endDate}'.`);
  if (wi.dayOfWeek !== OLD_DOW) throw new Error(`WI dayOfWeek drift: '${wi.dayOfWeek}'.`);

  ctx.log(`  WI ${wi.id.slice(0, 8)}: dates=${wi.startDate}→${wi.endDate}, dow=${wi.dayOfWeek}, weekOf=${wi.weekOf}`);
}

async function verify(ctx: MigrationContext): Promise<void> {
  const [wi] = await ctx.db.select().from(weekItems).where(eq(weekItems.title, WI_TITLE));
  if (!wi) throw new Error(`Post: WI disappeared.`);
  if (wi.startDate !== NEW_START) throw new Error(`Post: startDate '${wi.startDate}'.`);
  if (wi.endDate !== NEW_END) throw new Error(`Post: endDate '${wi.endDate}'.`);
  if (wi.dayOfWeek !== NEW_DOW) throw new Error(`Post: dayOfWeek '${wi.dayOfWeek}'.`);
  if (wi.weekOf !== WI_WEEK_OF) throw new Error(`Post: weekOf drift '${wi.weekOf}'.`);
  ctx.log(`  Verified: dates ${NEW_START}→${NEW_END}, dow ${NEW_DOW}, weekOf intact`);
}
...
async function snapshot(ctx: MigrationContext, relativePath: string): Promise<void> {
  ...
  const payload = {
    ...
    note: relativePath.includes("pre") ? "pre-thursday-move" : "post-thursday-move",
    client,
